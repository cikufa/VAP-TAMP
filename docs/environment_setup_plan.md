...
| Simulation VLM | Released prompt/parser in `GPT4VAgent`; modular Gemini/OpenAI transport; active `gemini-3.6-flash` free-tier substitution | Gemini inline-image validation passes; OpenAI remains restorable |
...
6. Record the approved provider/model deviation. Load credentials only from environment or
