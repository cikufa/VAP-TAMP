...
| Video/logging works | PASS for scripted replay; VLM logging pending | First/third-person MP4s decode, with action/position traces. Full VLM episode artifacts still require authentication |
...
1. Supply the user's real API key through the private setup command; a presence-only check still finds none. This is the current external blocker.
2. Authenticate the exact model and run one debug trial on the alternative released task, diagnosing any newly exposed verification/replanning issue before a pilot.
